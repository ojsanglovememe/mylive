...
import com.src.isec.data.utils.ZipHelper;
 ...
 * @class 解析框架中的网络请求和响应结果, 并以日志形式输出, 调试神器
 * 可使用 GlobalConfigModule.Builder # printHttpLogLevel(Level)}
 * 控制或关闭日志
...
public class RequestInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        boolean logRequest = printLevel == Level.ALL || (printLevel != Level.NONE && printLevel
                == Level.REQUEST);
        ...
        boolean logResponse = printLevel == Level.ALL || (printLevel != Level.NONE && printLevel
                == Level.RESPONSE);
        ...
        if (logResponse) {
            ...
            if (responseBody != null && isParseable(responseBody.contentType())) {
                mPrinter.printJsonResponse(TimeUnit.NANOSECONDS.toMillis(t2 - t1), isSuccessful,
                        code, header, responseBody.contentType(), bodyString, segmentList,
                        message, url);
            } else {
    ...
    /**
     *  @author liujiancheng
     *  @time 2018/3/19  17:03
     *  @describe 解析请求服务器的请求参数
     *  @param request  请求
    ...
    public static String parseParams(Request request) throws UnsupportedEncodingException {
        try {
            ...
            return CharacterHandler.jsonFormat(URLDecoder.decode(requestbuffer.readString
