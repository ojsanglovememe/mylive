 ...
 * @time 2018/3/20 11:25
 * @change
 * @chang time
 * @class describe
 */

public class GlobalComponentCallbacks implements ComponentCallbacks2 {


    private Application mApplication;

    @Inject
    public GlobalComponentCallbacks(Application application) {
     ...
     * @param level 内存级别
     * @author liujiancheng
     * @time 2018/3/20  11:29
     * @describe 在 App 生命周期的任何阶段, {@link ComponentCallbacks2#onTrimMemory(int)}
     * 发生的回调都预示着你设备的内存资源已经开始紧张
     * 应该根据 {@link ComponentCallbacks2#onTrimMemory(int)} 发生回调时的内存级别来进一步决定释放哪些资源
     * {@link ComponentCallbacks2#onTrimMemory(int)} 的回调可以发生在
     * {@link Application}、{@link Activity}、{@link Service}、{@link ContentProvider
     * }、{@link Fragment}
     * @see
     * <a href="https://developer.android.com/reference/android/content/ComponentCallbacks2.html#TRIM_MEMORY_RUNNING_MODERATE">level 官方文档</a>
     */
    @Override
...
//                case TRIM_MEMORY_RUNNING_MODERATE:
        ...
        //设备运行更缓慢了, 不会被 kill, 但请你回收 unused 资源, 以便提升系统的性能, 你应该释放不用的资源用来提升系统性能 (但是这也会直接影响到你的
        // App 的性能)
//                case TRIM_MEMORY_RUNNING_LOW:


        //设备运行特别慢, 当前 App 还不会被杀死, 但是系统已经把 LRU 列表中的大多数进程都已经杀死, 因此你应该立即释放所有非必须的资源
        //如果系统不能回收到足够的 RAM 数量, 系统将会清除所有的 LRU 列表中的进程, 并且开始杀死那些之前被认为不应该杀死的进程, 例如那个包含了一个运行态
        // Service 的进程
...
//                case TRIM_MEMORY_COMPLETE:
    ...
    /**
     * @author liujiancheng
     * @time 2018/3/20  11:30
     * @describe 设备配置信息发送改变时的回调，如屏幕旋转
     */
    @Override
    ...
    /**
     * @author liujiancheng
     * @time 2018/3/19  15:32
     * @describe * 当系统开始清除 LRU 列表中的进程时, 尽管它会首先按照 LRU 的顺序来清除, 但是它同样会考虑进程的内存使用量, 因此消耗越少的进程则越容易被留下来
