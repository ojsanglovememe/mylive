    ...
    private void faceAttributeDetect(byte[] data, STHumanAction humanAction){
        if (humanAction != null && data != null && data.length == mImageWidth * mImageHeight * 3/2) {
            ...
            if (arrayFaces != null && arrayFaces.length != 0) { // face attribute
                ...
                int result = mSTFaceAttributeNative.detect(data, STCommon.ST_PIX_FMT_NV21, mImageHeight,
                        mImageWidth, arrayFaces, arrayFaceAttribute);
...
//                LogUtils.i(TAG, "human action cost time: %d", System.currentTimeMillis() - startHumanAction);
                ...
                if(DEBUG){
                    if(humanAction != null && humanAction.faceCount > 0){
                        ...
                        mFaceExpressionResult = mSTHumanActionNative.getExpression(humanAction, getHumanActionOrientation(), mCameraID == Camera.CameraInfo.CAMERA_FACING_FRONT);
//                        LogUtils.i(TAG, "face expression cost time: %d", System.currentTimeMillis() - expressionStartTime);
                    ...
                    if(mFrameCount <= 20){
                        ...
                        mFrameCount = 0;
                        faceAttributeDetect(mNv21ImageData, humanAction);//do face attribute
                    }
                }
                ...
                if (mNeedBeautify) {// do beautify
                    ...
                    if (!mNeedBeautyOutputBuffer) {
                        ...
                        result = mStBeautifyNative.processTextureAndOutputBuffer(textureId, mImageWidth, mImageHeight, humanAction, mBeautifyTextureId[0], outputImageBuffer, STCommon.ST_PIX_FMT_RGBA8888, mHumanActionBeautyOutput);
                    ...
                    */
                    ...
                    if (!mNeedStickerOutputBuffer) {
                        ...
                        result = mStStickerNative.processTextureAndOutputBuffer(textureId, humanAction, orientation, mImageWidth,
                                mImageHeight, frontStickerOrientation, false, mTextureOutId[0], STCommon.ST_PIX_FMT_RGBA8888, outputImageBuffer);
...
//                            LogUtils.i(TAG, "human action body[0] action: %d", humanAction.bodys[0].bodyAction);
            ...
            if(mNeedFilter){
                ...
                if (!mNeedFilterOutputBuffer) {
                    ...
                    byte[] outputImageBuffer = new byte[mImageWidth * mImageHeight * 4];
                    result = mSTMobileStreamFilterNative.processTextureAndOutputBuffer(textureId, mImageWidth, mImageHeight, mFilterTextureOutId[0], outputImageBuffer, STCommon.ST_PIX_FMT_RGBA8888);
